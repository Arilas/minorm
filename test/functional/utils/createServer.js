/** @flow */
import {
  createServer as createFakeServer,
  type FakeServerConnection,
} from 'mysql2'
...
function parseWhere(whereClause) {
  ...
  switch (modifier) {
    ...
    default: {
      const value =
        parseInt(where, 10).toString() == where ? parseInt(where, 10) : where
      return {
        // $FlowIgnore
        [modifier]: value,
      }
...
export function createServer() {
  ...
  server.on(
    'connection',
    (conn: FakeServerConnection): void => {
        ...
        if (sql.indexOf('INSERT') !== -1) {
          ...
          const into = {
            ...
            structure: table
              .split('(')[1]
              .split(',')
              .map(col => col.replace(specialRegex, '').trim()),
          }
          const rows = rawValues.split('),').map(line =>
            line
          ...
          for (const row of rows) {
            database[into.tableName] = database[into.tableName].set(
              database[into.tableName].size + 1,
              {
                ...row,
          ...
          const values = rawValues.split(',').reduce(
            (target, value) =>
              Object.assign(target, {
              ...
              (target, part) =>
                Object.assign(target, {
                  [part
                    .trim()
                    .split(' ')[0]
                    .trim()]: parseWhere(part),
                }),
              {},
            )
          const matcher = makeQueryFilter(where)
          const matched = [...database[into]].filter(val =>
            matcher.match(val[1]),
          )
