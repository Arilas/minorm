...
import { createSchemaTool, SchemaToolContext } from '@minorm/schema-tool'
...
describe('Integration', () => {
  describe('Schema', () => {
    test('should init Database schema', async () => {
      ...
      schemaTool.getMigrationManager().addMigration('2016-11-16 18:04:24', {
        up(schema) {
      ...
      // First migration
      schemaTool.getMigrationManager().addMigration('2016-11-16 17:42:15', {
        up(schema) {
          schema.use('users', table => {
    ...
    test('should apply changes when I directly ask for this', async () => {
      ...
      schemaTool.setSchemaInit({
        up: function*(schema) {
          ...
          const [users] = yield schema.asyncQuery(
