...
export function createRepository<T: BaseRecord>(
  ...
  const repo = {
    ...
    find(id: number): Promise<Model<T> | null> {
      const query: SelectQuery<T> = this.startQuery().where('id = ?', id)
      return query
        .execute()
        .then(
          (result: Array<T>): Model<T> | null =>
            result.length > 0 ? this.hydrate(result.pop(), true) : null,
        )
    },
    ...
    findBy(
      ...
      const query: SelectQuery<T> = this.startQuery().criteria(criteria)
      if (limit) {
        query.limit(limit)
      }
      if (offset) {
      ...
      return query
        .execute()
        .then(
          (result: Array<T>): Array<Model<T>> =>
            result.map((entry: T): Model<T> => this.hydrate(entry, true)),
        )
    },
